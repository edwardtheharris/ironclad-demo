...
# app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://ironclad:password@pg.pg.svc.cluster.local:5432/ironclad'
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://ironclad:ironclad-is-copper-bottom@192.168.49.2:5432/ironclad'
...
def add_user():
    ...
    new_user = User(first_name=first_name, middle_name=middle_name, last_name=last_name,
                    email=email, phone_number=phone_number, date_of_birth=dob)
...
def update_user(user_id):
    ...
    date_of_birth = data.get('date_of_birth', user.date_of_birth.strftime('%m/%d/%Y')).strip()
